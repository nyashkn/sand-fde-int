...
openspec as of this document. The two statistical checks that `design.md` calls
"structural guards" in its own spec prose (`3.1` temporal-signal guard, `3.3`
stratification guard) **are** built and running, under different names than the spec
still uses: ADR 0012 renamed `guards.py` to `checks.py` and `temporal_signal_guard` /
`stratification_guard` to `temporal_signal_check` / `stratification_check`, and changed
what they do on a failed bar from refusing to render (a "Withheld" section) to always
rendering with a stated caveat. The permutation test, the stratified-correlation test,
the fixed seed and trial count are byte-for-byte unchanged from what `design.md`
describes; only the consumer contract and the vocabulary drifted, and `data-validation`'s
own spec text has not been updated to match. So the change is partially, not wholly,
unimplemented, and this register does not claim otherwise; it is the conflicts
table/triage queue specifically, not the whole `data-validation` change, that remains
unbuilt.
...
mechanism. ADR 0012's removal of the withhold gate for the statistical checks is the same
principle applied a second time, to a different gate: the run always completes, and what
would have been silence is now a disclosed caveat instead.
