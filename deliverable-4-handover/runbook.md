...
bun run publish                           # build all four quarters, publish to ../output
```
...
Six steps run, in this order: `check-tokens.mjs`, the Astro build itself, `email.mjs`
(the inliner), `check-style.mjs`, `check-email.mjs`, `check-agreement.mjs`. **The four
named checks** (`check-tokens.mjs`, `check-style.mjs`, `check-email.mjs`,
`check-agreement.mjs`, the same four `deliverable-2-prototype/README.md` lists) **must
each print `passed`**; the build and the inliner must simply complete without erroring.
If any step fails, stop. Do not publish a bulletin whose checks failed.
...
All four quarters (`2024-Q1` through `2024-Q4`) publish on every run; there is no
per-quarter flag to build one in isolation.
...
| `check-email.mjs` | The email edition exceeds roughly 71 KB (a warning, 70% of the clip ceiling) or 102 KB (a failure, Gmail's clip threshold), embeds an `<svg>` chart, uses `flex`/`grid` layout Outlook cannot render, or is missing the words "provisional"/"caveat". |
| `check-agreement.mjs` | The bulletin and the email edition state a shared figure, deaths, live births, months held, provisional count, or one of the two statistical caveats, differently. This has happened once for real: fix the mart, not the template. |
...
`bun run verify` prints four `passed` lines. The pipeline was measured byte-for-byte
reproducible on 2026-08-11: two clean runs produced the identical hash
...
**That measurement predates ADR 0012** (withhold-gate removal, four-quarter publish, four
new gold-layer tables, the Flint chart rewrite) and no longer applies to the current
commit; the committed `output/bulletin-2024-Q1.html` today hashes to
`2bafa326c8925bfe508bc337e97e9e6e`, a different document on purpose. The determinism
*claim* was not re-run against the post-ADR-0012 pipeline as part of this pass, to avoid
writing build output into `deliverable-2-prototype/` while other work is in flight there.
Nothing in ADR 0012 introduces a new source of run-to-run variance, so determinism is
carried over by design, but it is unverified until someone runs this section twice and
diffs the hashes. Do that before citing this runbook as proof of reproducibility again.

This was run against a prior version of this document on 2026-08-11 from a fully wiped
state, with no step added, removed, or reordered from what is written above. See
`deliverable-4-handover/README.md`, "Verification", for the result and the staleness note.
